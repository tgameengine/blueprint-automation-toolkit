...
void FBlueprintAutomationToolkitModule::BindUObjectRoutes()
{
	...
	UObjectSetRoute = Router->BindRoute(
		...
		FHttpRequestHandler::CreateLambda([this](const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
		{
			...
			AsyncTask(ENamedThreads::GameThread, [this, Path, Values, RequestId, OnComplete]()
			{
				UObject* TargetObject = ResolveObjectFromPath(Path);
				if (!TargetObject)
				{
				...
				if (!Values.IsValid())
				{
					OnComplete(MakeErrorResponse(EHttpServerResponseCodes::BadRequest, RequestId, TEXT("bad_args"), TEXT("Body must include object field 'values'")));
					return;
				}
				...
				for (const TPair<FString, TSharedPtr<FJsonValue>>& Pair : Values->Values)
				{
					...
					{
						OnComplete(MakeErrorResponse(EHttpServerResponseCodes::BadRequest, RequestId, DecodeCode, FString::Printf(TEXT("%s (%s)"), *DecodeMessage, *PropertyPath)));
						return;
					}
				}
	...
	UObjectCallRoute = Router->BindRoute(
		...
		FHttpRequestHandler::CreateLambda([this](const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
		{
			...
			AsyncTask(ENamedThreads::GameThread, [this, Path, FunctionName, ArgsObj, RequestId, OnComplete]()
			{
				UObject* TargetObject = ResolveObjectFromPath(Path);
				if (!TargetObject)
				{
				...
				if (!AllowedUObjectFunctions.Contains(FunctionFName))
				{
					UE_LOG(LogBlueprintAutomationToolkitUObject, Warning, TEXT("Denied /uobject/call function '%s': not in allowlist"), *FunctionName);
					OnComplete(MakeErrorResponse(EHttpServerResponseCodes::Denied, RequestId, bSafeModeEnabled ? TEXT("safe_mode_denied") : TEXT("call_denied"), TEXT("Function is not allowlisted")));
					return;
				}
				...
				if (!Function)
				{
					OnComplete(MakeErrorResponse(EHttpServerResponseCodes::NotFound, RequestId, TEXT("function_not_found"), TEXT("Function not found on target object")));
					return;
				}
				...
				{
					UE_LOG(LogBlueprintAutomationToolkitUObject, Warning, TEXT("Denied /uobject/call function '%s': flags disallowed"), *FunctionName);
					OnComplete(MakeErrorResponse(EHttpServerResponseCodes::Denied, RequestId, TEXT("call_denied"), TEXT("Function flags are not allowed")));
					return;
				}
				...
				for (TFieldIterator<FProperty> It(Function); It && (It->PropertyFlags & CPF_Parm); ++It)
				{
					...
					if (!JsonField)
					{
						OnComplete(MakeErrorResponse(EHttpServerResponseCodes::BadRequest, RequestId, TEXT("bad_args"), FString::Printf(TEXT("Missing argument '%s'"), *ParamProperty->GetName())));
						return;
					}
					...
					{
						OnComplete(MakeErrorResponse(EHttpServerResponseCodes::BadRequest, RequestId, DecodeCode, FString::Printf(TEXT("Invalid argument '%s': %s"), *ParamProperty->GetName(), *DecodeMessage)));
						return;
					}
				}
	...
	ActorSpawnRoute = Router->BindRoute(
		...
		FHttpRequestHandler::CreateLambda([this](const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
		{
			...
			AsyncTask(ENamedThreads::GameThread, [this, ClassPath, Name, LocationFieldCopy = LocationField ? *LocationField : TArray<TSharedPtr<FJsonValue>>(), RotationFieldCopy = RotationField ? *RotationField : TArray<TSharedPtr<FJsonValue>>(), ScaleFieldCopy = ScaleField ? *ScaleField : TArray<TSharedPtr<FJsonValue>>(), PropertiesObj, RequestId, OnComplete]()
			{
				...
				{
					OnComplete(MakeErrorResponse(EHttpServerResponseCodes::BadRequest, RequestId, TEXT("type_unsupported"), TEXT("'class' must resolve to an Actor class")));
					return;
				}
				...
				if (LocationFieldCopy.Num() > 0 && !TryParseVec3(MakeShared<FJsonValueArray>(LocationFieldCopy), SpawnLocation))
				{
					OnComplete(MakeErrorResponse(EHttpServerResponseCodes::BadRequest, RequestId, TEXT("bad_args"), TEXT("'location' must be [x,y,z]")));
					return;
				}
				if (RotationFieldCopy.Num() > 0 && !TryParseRotator(MakeShared<FJsonValueArray>(RotationFieldCopy), SpawnRotation))
				{
					OnComplete(MakeErrorResponse(EHttpServerResponseCodes::BadRequest, RequestId, TEXT("bad_args"), TEXT("'rotation' must be [pitch,yaw,roll]")));
					return;
				}
				if (ScaleFieldCopy.Num() > 0 && !TryParseVec3(MakeShared<FJsonValueArray>(ScaleFieldCopy), SpawnScale))
				{
					OnComplete(MakeErrorResponse(EHttpServerResponseCodes::BadRequest, RequestId, TEXT("bad_args"), TEXT("'scale' must be [x,y,z]")));
					return;
				}
				...
				if (!SpawnedActor)
				{
					OnComplete(MakeErrorResponse(EHttpServerResponseCodes::ServerError, RequestId, TEXT("not_found"), TEXT("Failed to spawn actor")));
					return;
				}
				...
				if (PropertiesObj.IsValid())
				{
					for (const TPair<FString, TSharedPtr<FJsonValue>>& Pair : PropertiesObj->Values)
					{
						...
						if (PropertyPath.Equals(TEXT("ActorLabel"), ESearchCase::IgnoreCase))
						{
							...
							{
								OnComplete(MakeErrorResponse(EHttpServerResponseCodes::BadRequest, RequestId, TEXT("bad_args"), TEXT("ActorLabel requires a string")));
								return;
							}
							SpawnedActor->SetActorLabel(Pair.Value->AsString());
							continue;
						}
#endif
						if (!IsPropertyPathAllowed(AllowedUObjectProperties, PropertyPath))
						...
						if (!DeserializePropertyValue(Resolved.Property, Resolved.ValuePtr, Pair.Value, DecodeCode, DecodeMessage))
						{
							OnComplete(MakeErrorResponse(EHttpServerResponseCodes::BadRequest, RequestId, DecodeCode, FString::Printf(TEXT("Invalid property '%s': %s"), *PropertyPath, *DecodeMessage)));
							return;
						}
					}
				}
