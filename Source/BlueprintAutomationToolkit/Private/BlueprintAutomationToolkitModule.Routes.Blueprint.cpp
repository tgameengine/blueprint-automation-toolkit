...
BlueprintGraphsRoute = Router->BindRoute(
	...
	BlueprintGraphLinksRoute = Router->BindRoute(
		...
		FHttpRequestHandler::CreateLambda([this](const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
		{
			...
			const bool bCompleted = RunOnGameThreadWait([this, &DataValue, BlueprintPath, GraphName]()
			{
				UBlueprint* Blueprint = nullptr;
				FString ObjectPath;
	...
	BlueprintGraphApplyRoute = Router->BindRoute(
		...
		FHttpRequestHandler::CreateLambda([this](const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
		{
			...
			TArray<FString> ParseErrors;
			if (!BAT::BlueprintGraphApplyRequest::Parse(BodyObj, ParsedRequest, ParseErrors))
			{
				const FString RequestId = ResolveOrCreateRequestId(Request);
				...
				OnComplete(BAT::Http::MakeJsonResponse((int32)EHttpServerResponseCodes::BadRequest, ErrorObj, RequestId));
				return true;
			}

			const FString RequestId = ResolveOrCreateRequestId(Request);

			FBlueprintService Service([this, ParsedRequest](FAutomationContext& Context) -> FAutomationResult
			{
				TOptional<FAutomationResult> ThreadResult;
				const bool bCompleted = RunOnGameThreadWait([ParsedRequest, &ThreadResult]()
				{
					ThreadResult = FBlueprintGraphService::ApplyGraphPatch(ParsedRequest);
				}, 10.0);
				if (!bCompleted)
				{
					return FAutomationResult::Error(TEXT("game_thread_timeout"), TEXT("Timed out waiting for GameThread execution"), 504);
				...
				if (!ThreadResult.IsSet())
				{
					return FAutomationResult::Error(TEXT("internal_error"), TEXT("No result produced by graph apply operation"), 500);
				}
			...
			if (!Result.bSuccess)
			{
				if (Result.Data.IsValid() && Result.Data->Type == EJson::Object)
				{
					OnComplete(BAT::Http::MakeJsonResponse(Result.StatusCode, Result.Data->AsObject().ToSharedRef(), RequestId));
				}
				else
				{
					OnComplete(MakeErrorResponse(Result.StatusCode, RequestId, Result.ErrorCode, Result.ErrorMessage));
			...
			if (!Result.Data.IsValid() || Result.Data->Type != EJson::Object)
			{
				OnComplete(MakeErrorResponse(500, RequestId, TEXT("internal_error"), TEXT("Apply graph patch returned invalid payload")));
				return true;
			}
	...
	BlueprintNodeDescribeRoute = Router->BindRoute(
		FHttpPath(TEXT("/blueprint/node/describe")),
		EHttpServerRequestVerbs::VERB_POST,
		FHttpRequestHandler::CreateLambda([this](const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
		{
			if (!ValidateAndHandleRequest(Request, OnComplete, TEXT("/blueprint/node/describe")))
			{
				return true;
			}
